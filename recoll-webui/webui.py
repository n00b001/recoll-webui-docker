...
def get_config():
    ...
    topdirs = (
        rclconf.config.get("topdirs", rclconf.keydir)
        if rclconf.config
        else rclconf.keydir
    )
    config["dirs"] = [
        os.path.expanduser(d) for d in shlex.split(topdirs or rclconf.keydir or "")
    ]
    config["stemlang"] = (
        rclconf.config.get("indexstemminglanguages", rclconf.keydir)
        if rclconf.config
        else (rclconf.keydir or "")
    )
...
def set():
    ...
    for k, v in DEFAULTS.items():
        bottle.response.set_cookie(
            k, str(bottle.request.query.get(k)), max_age=3153600000, expires=315360000
        )
    for d in config["dirs"]:
        cookie_name = "mount_%s" % urllib.quote(d, "")
        bottle.response.set_cookie(
            cookie_name,
            str(bottle.request.query.get("mount_%s" % d)),
            max_age=3153600000,
            expires=315360000,
        )
    bottle.redirect("./")
